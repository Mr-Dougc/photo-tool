...
async function getAccessToken() {
  ...
  const { data } = await axios.post('https://aip.baidubce.com/oauth/2.0/token', null, {
    params: { grant_type: 'client_credentials', client_id: process.env.BAIDU_API_KEY, client_secret: process.env.BAIDU_SECRET_KEY },
    timeout: 10000
  })
...
app.post('/api/remove-bg', async (req, res) => {
  try {
    ...
    const { data } = await axios.post('https://aip.baidubce.com/rest/2.0/image-classify/v1/body_seg', params.toString(), {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      timeout: 60000
    })
    if (data.error_code) return res.status(400).json({ error: data.error_msg })
